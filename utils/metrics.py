...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, SCA=False, alpha = 1.0, eps=1e-7):
    ...
    if SCA:
        ...
        d_lt = (b1_x1 - b2_x1 + eps)**2 +\
                (b1_y1 - b2_y1 + eps)**2
        d_rb = (b1_x2 - b2_x2 + eps)**2 +\
                (b1_y2 - b2_y2 + eps)**2
        d_diag = (max_x2 - max_x1 + eps)**2 +\
                (max_y2 - max_y1 + eps)**2

        CD = (torch.pow(d_lt, 0.5) / (torch.pow(d_diag, 0.5) + eps) +\
                torch.pow(d_rb, 0.5) / (torch.pow(d_diag, 0.5) + eps)) * alpha
